...
resource "aws_route53_record" "frontend_alb" {
  zone_id = var.zone_id
  name    = "roboshop-${var.environment}.${var.domain_name}" # roboshop-dev.deepthi.cloud
  type    = "A"
  allow_overwrite = true

  alias {
    # These are ALB Details,not our domain details.
    name                   = aws_lb.frontend_alb.dns_name # aws_elb.frontend_alb.dns_name comes from your ALB resource.Public DNS name AWS assigns to your Load Balancer.
